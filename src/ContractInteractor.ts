...
export class ContractInteractor {
  ...
  async _createRelayVerifier (address: string): Promise<IRelayVerifierInstance> {
    return await this.IRelayVerifierContract.at(address);
  }

  async _createDeployVerifier (address: string): Promise<IDeployVerifierInstance> {
  ...
  async estimateDestinationContractCallGas (transactionDetails: EstimateGasParams, addCushion = true): Promise<number> {
  ...
  async getBalance (address: string, defaultBlock: BlockNumber = 'latest'): Promise<string> {
  ...
  async walletFactoryDeployEstimateGasForInternalCall (request: DeployRequest, factory: string, domainHash: string,
    suffixData: string, signature: string, testCall = false): Promise<number> {
