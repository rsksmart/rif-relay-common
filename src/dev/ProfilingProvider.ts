...
export class ProfilingProvider extends WrapperProviderBase {
    ...
    constructor(provider: HttpProvider, logTraffic = false) {
        super(provider);
    ...
    send(payload: JsonRpcPayload, callback: SendCallback): void {
        ...
        if (this.logTraffic) {
            ...
            console.log(
                `>>> payload: ${JSON.stringify(payload) ?? 'null result'}`
            );
        }
    ...
    log(): void {
        ...
        new Map(
            [...this.methodsCount.entries()].sort(function (
                [, count1],
                [, count2]
            ) {
            ...
            console.log(
                `Method: ${key.padEnd(30)} was called: ${value
                    .toString()
                    .padEnd(3)} times`
            );
        });
